...
AFS provides the users/developers can upload their analytics to the catalog. For other users, if they find the analytics in the catalog are capable for solving their problems, they can subcribe the analytics from catalog.    
...
3. The structure of gitlab repository is introduced.
4. Finally, there is an example to demonstrate the steps. 
...
* Naming rule of Repository Name
	...
	`{company_name}_{Repository_Name}`
	- The name shoule be start from **company name**. All of company characters are small letter. The first character of each word is capitalized, and connect by the bottom line betwwen words.
...
* We could .ipynb file directly in Jupyter Notebook.
* A description (less than 50 words) which shows in the catalog index page is required.
